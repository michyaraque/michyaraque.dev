...
export const getFiles = (securePath: string) =>
  fs.readdirSync(path.join(root, `data/${securePath}`));

export const getFileBySlug = async (slug: string, securePath: string) => {
  const mdxSource = fs.readFileSync(
    path.join(root, `data/${securePath}`, `${slug}.mdx`),
    "utf8"
  );
...
export const getAllFilesMetadata = (securePath: string) => {
  const files = getFiles(securePath).filter(
    (file) => file !== "_configuration.mdx"
  );
  return files.reduce(
    (allPosts: Array<Record<string, any>>, postSlug: string) => {
      const mdxSource = fs.readFileSync(
        path.join(root, `data/${securePath}`, postSlug),
        "utf8"
      );
...
export const getCoursesPath = () => {
  const coursePaths = fs
    .readdirSync(path.join(root, `data/courses`), { withFileTypes: true })
...
export const getAllFilesMetadataByCourseModules = (securePath: string) => {
  ...
  return files.reduce(
    (allPosts: Array<Record<string, string>>, postSlug: string) => {
      const mdxSource = fs.readFileSync(
        path.join(root, `data/${securePath}`, postSlug),
        "utf8"
      );
...
export const getCourseMetadata = async (course: string) => {
  const mdxSource = fs.readFileSync(
    path.join(root, `data/courses/${course}`, "_configuration.mdx"),
    "utf8"
  );
